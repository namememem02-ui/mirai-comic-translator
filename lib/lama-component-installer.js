...
function createLamaComponentInstaller(options = {}) {
  ...
  const rawFs = options.fs || defaultFs;
  const fsApi = rawFs && rawFs.promises ? rawFs.promises : rawFs;
    ...
    try {
      ...
      if (!response || response.ok !== true || !response.body) {
        if (response && response.status === 404) {
          throw new Error('ไม่พบไฟล์ส่วนประกอบ LaMa Component บน GitHub Releases (HTTP 404)');
        }
        const statusStr = response ? `HTTP ${response.status}` : 'เชื่อมต่อไม่ได้';
        throw new Error(`ดาวน์โหลดส่วนประกอบ LaMa ไม่สำเร็จ (${statusStr})`);
      }
