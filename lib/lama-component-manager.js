...
function safeHardware(value) {
  ...
  if (typeof nvidiaValue.name === 'string' && nvidiaValue.name.length <= 100) {
    nvidia.name = nvidiaValue.name;
  }
  ...
  if (nvidia.name) hw.nvidiaName = nvidia.name;
  if (typeof value.nvidiaAvailable === 'boolean') hw.nvidiaAvailable = value.nvidiaAvailable;
  return hw;
}
...
function createLamaComponentManager(options = {}) {
  ...
  function project(candidate) {
    ...
    if (projected.componentVersion) next.installedVersion = projected.componentVersion;
    ...
    for (const backend of BACKENDS) {
      try {
        ...
        if (metadata) {
          ...
          const sidecarAdapter = adapterFor(backend);
          if (sidecarAdapter && typeof sidecarAdapter.getStatus === 'function') {
            const sidecarStatus = sidecarAdapter.getStatus();
            if (sidecarStatus && sidecarStatus.state === 'ready') {
              const actualBackend = (sidecarStatus.backend === 'cuda' || sidecarStatus.backend === 'nvidia') ? 'nvidia' : 'cpu';
              const rawVer = sidecarStatus.componentVersion;
              const cleanVer = typeof rawVer === 'string' && /^\d+\.\d+\.\d+$/.test(rawVer.trim()) ? rawVer.trim() : '1.0.0';
              inventory.set(actualBackend, {
  ...
  async function startAdapter(backend) {
    ...
    const cleanupLateSettlement = () => {
      if (closed) cleanupAdapter(adapter, backend).catch(() => {});
    };
    startup.then(cleanupLateSettlement, cleanupLateSettlement);
    try {
  ...
  async function installMutation(backend, explicitRecovery = false) {
    ...
    try {
      ...
      activated = true;
      const metadata = await abortable(inspect(backend, { root }), controller.signal) || result;
      ...
      if ((controller && controller.signal.aborted) || (error && error.name === 'AbortError')) {
        if (activated) {
          inventory.delete(backend);
          publish(selectedState({ allowNotInstalled: true }));
        } else {
          restore(previous);
        }
        throw abortError();
  ...
  return {
    ...
    shutdown() {
      ...
      shutdownPromise = enqueue(async () => {
        const adapterBackends = new Map();
        const associate = (adapter, backend) => {
          if (!adapter) return;
          if (!adapterBackends.has(adapter)) adapterBackends.set(adapter, new Set());
          if (BACKENDS.has(backend)) adapterBackends.get(adapter).add(backend);
        };
        associate(sidecar, activeBackend);
        if (sidecars) {
          for (const [backend, adapter] of Object.entries(sidecars)) associate(adapter, backend);
        }
        try {
          const actions = [];
          for (const [adapter, backends] of adapterBackends) {
            const shutdownAdapter = method(adapter, ['shutdown']);
            if (shutdownAdapter) {
              actions.push(() => shutdownAdapter());
              continue;
            }
            const stopAdapterForBackend = method(adapter, ['stop']);
            if (stopAdapterForBackend) {
              for (const backend of backends) actions.push(() => stopAdapterForBackend(backend));
            }
          }
