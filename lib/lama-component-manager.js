        ...
        if (metadata) {
          ...
          if (sidecarAdapter && typeof sidecarAdapter.getStatus === 'function') {
            ...
            if (sidecarStatus && sidecarStatus.state === 'ready') {
              const actualBackend = sidecarStatus.backend === 'cuda' ? 'nvidia' : (sidecarStatus.backend || 'cpu');
              if (actualBackend === backend || (backend === 'nvidia' && sidecarStatus.backend === 'cuda')) {
                inventory.set(backend, {
                  backend,
                  version: sidecarStatus.componentVersion || '1.0.0 (System)',
