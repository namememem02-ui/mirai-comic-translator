...
function createProjectBackupIpcCoordinator(deps) {
  ...
  function promoteBackup(tempPath, destination) {
    ...
    try {
      ...
      if (!oldMoved) {
        try { deps.fs.rmSync(backupPath, { force: true }); } catch (cleanupError) {
          logFailure('Project backup destination backup cleanup failure', cleanupError);
        }
      }
    }
  }
  ...
  async function backupProject(_event, args) {
    ...
    try {
      ...
      promoteBackup(tempPath, result.filePath);
      tempPath = null;
      return {
        success: true,
