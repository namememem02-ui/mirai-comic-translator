  ...
  return {
    manifest: {
      ...
      glossary: glossary ? {
        ...
        byteCount: files.find(file => file.archivePath === 'data/glossary.json').size,
      } : null,
      chapters,
...
function validateManifest(manifest) {
  ...
  if (!Object.prototype.hasOwnProperty.call(manifest, 'glossary')) throw new Error('Invalid glossary declaration');
  if (manifest.glossary !== null) {
    const glossary = manifest.glossary;
    if (!glossary || typeof glossary !== 'object' || Array.isArray(glossary)
      || Object.keys(glossary).length !== 2 || glossary.path !== 'data/glossary.json'
      || !Number.isSafeInteger(glossary.byteCount) || glossary.byteCount < 0) {
      throw new Error('Invalid glossary declaration');
    }
    declared.set('data/glossary.json', {
      path: 'data/glossary.json', kind: 'glossary', expectedBytes: glossary.byteCount,
    });
  }
  let imageCount = 0;
...
async function inspectProjectBackup(buffer, limits = {}) {
  ...
  for (const item of files) {
    ...
    if (declaration.kind === 'glossary' && item.size !== declaration.expectedBytes) {
      throw new Error('Glossary byte count mismatch');
    }
