...
const SAFE_REASONS = new Set([
  ...
  'gpu-inference-failed', 'cuda-out-of-memory', 'startup-failed', 'integrity-failed', 'unavailable',
]);
...
function validateComponentManifest(value, options = {}) {
  ...
  const packages = value.packages.map((entry) => {
    ...
    if (entry.backend === 'nvidia' && (typeof entry.minDriver !== 'string'
      || entry.minDriver.length > MAX_DRIVER_LENGTH || !DRIVER_PATTERN.test(entry.minDriver))) {
      throw manifestError();
    }
...
function selectLamaBackend(input) {
  ...
  if (settings.mode === 'auto') {
    if (candidate.gpuFailure) return unavailableNvidia(candidate, settings);
...
function validStatusInvariants(value) {
  const requiredBackend = {
    'ready-cpu': 'cpu',
    'ready-nvidia': 'nvidia',
    'gpu-fallback': 'cpu',
  }[value.state];
  if (requiredBackend && value.backend !== requiredBackend) return false;
  if (value.state === 'cpu-download-required' && value.backend !== undefined) return false;

  const hasProgress = hasOwn(value, 'receivedBytes') || hasOwn(value, 'totalBytes');
  if (value.state === 'downloading' && !hasProgress) return false;
...
function safeComponentStatus(value) {
  if (!isPlainObject(value) || !STATES.has(value.state) || !validStatusInvariants(value)) {
    return safeStatusError();
  }
  ...
  if (value.state === 'error') status.message = 'AI retouch unavailable';
  if (isPlainObject(value.diagnostic)) {
    const code = boundedString(value.diagnostic.code, 100);
    if (code && SAFE_REASONS.has(code)) status.diagnostic = { code };
  }
  return status;
}
